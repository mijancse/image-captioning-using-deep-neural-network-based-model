...
directory = 'COCO'
filename = directory +'/text/p_train_images.txt'
...
# load validation set
filename = directory+'/text/p_validation_images.txt'
...
#model = load_model(directory+'/models/prev/model_9.h5')
# train the model, run epochs manually and save after each epoch
epochs = 10
image_samples = 8
steps = int(len(train_descriptions)/image_samples)
validation_steps = int(len(validation_descriptions)/image_samples)
...
for i in range(epochs):
    ...
    if best_model is None or loss < min_loss:
        best_model = model
        min_loss = loss
        best_model.save(directory + '/models/best_model_8.h5')
    # save model
    model.save(directory+'/models/model8_' + str(i) + '.h5')


model = best_model

epochs = 10
image_samples = int(2*image_samples)
steps = int(len(train_descriptions)/image_samples)
validation_steps = int(len(validation_descriptions)/image_samples)
...
for i in range(epochs):
    ...
    if best_model is None or loss < min_loss:
        best_model = model
        min_loss = loss
        best_model.save(directory + '/models/best_model_person.h5')
    # save model
    model.save(directory+'/models/model_p_' + str(i) + '.h5')
